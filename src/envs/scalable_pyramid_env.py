...
class ScalablePyramidEnv(Env):
    """Gymnasium wrapper around :class:`ScalablePyramidCore`."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Compatibility aliases for the newer project structure.
        self.core = self.env
    ...
    def reset(
        ...
        *,
        ...
        if seed is not None:
            self.seed(seed)
            # Keep task structure fixed unless the caller explicitly rewrites seeds
            # in the config. Resetting here should only affect episode-level randomness.
            self.env.general_rng = np.random.default_rng(int(seed))
        obs = self.env.reset()
        self.current_node_idx = self.env.current_node_idx
        ...
        info = {
            ...
            "episode_step": 0,
            "external_reward": 0.0,
            "total_reward": float(self.env.total_reward),
            "action": None,
        }
...
# ------------------------------------------------------------------
# Compatibility aliases for the new project layout
# ------------------------------------------------------------------
