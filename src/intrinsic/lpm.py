...
class LPMIntrinsicReward:
    ...
    def compute(
        ...
        *,
        ...
        warmup_active = len(self.error_buffer) < warmup
        mse_signed = 0.0
        if not warmup_active:
            mse_signed = float(self.config.expected_error_scale) * expected_error - actual_error
        lpm_signed = predicted_log_error - current_log_error
        raw_used, normalized_reward = self._postprocess_reward(mse_signed)

        self.last_actual_error = actual_error
        self.last_expected_error = expected_error
        self.last_raw_intrinsic = mse_signed
        info_values = info or {}
    ...
    def _update_error_model(self) -> Optional[dict[str, float]]:
        ...
        *,
        ...
        if self._model_update_count % error_update_interval == 0:
            for _ in range(max(int(self.config.error_updates), 0)):
                metrics = self._update_error_model()
